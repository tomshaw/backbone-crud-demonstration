...
window.CustomerListView = Backbone.View.extend({
    ...
    render: function (event) {
    	$(this.el).html(this.template({customers:this.model.toJSON()}));
    	$(this.el).prepend(new PaginatorTemplate({model: this.model}).render().el);
        return this;
    } 
...
window.PaginatorTemplate = Backbone.View.extend({
    ...
    render:function (event) {
    	$(this.el).html(this.template({ data: this.model.pages }));
        return this;
    }
});
...
var AppRouter = Backbone.Router.extend({
    ...
    home: function () {
        ...
        customerListCollection.fetch({
            success: function() {
                $("#content").html(new CustomerListView({model: customerListCollection}).render().el);
            }
        });
